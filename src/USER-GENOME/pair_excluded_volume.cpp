...
//                        sin(arg) * MY_PI/cut[itype][jtype]/r;
        if (r> 0.0) fpair = factor_lj * prefactor[itype][jtype] * sin(arg) * cut[itype][jtype] / MY_PI / r;
        ...
        if (eflag)
          ...
          evdwl = factor_lj * prefactor[itype][jtype] * cutsq[itype][jtype] * (1.0+cos(arg)) / MY_PI / MY_PI;
...
void PairExcludedVolume::settings(int narg, char **arg)
{
  ...
  prefactor_global = force->numeric(FLERR,arg[0]);
  ...
  if (allocated) {
    ...
    for (i = 1; i <= atom->ntypes; i++)
      for (j = i; j <= atom->ntypes; j++)
        if (setflag[i][j]) prefactor[i][j] = prefactor_global;
  }
}
...
void PairExcludedVolume::coeff(int narg, char **arg)
{
  ...
  double *radius = atom->dvector[index];
...
{
  fwrite(&prefactor_global,sizeof(double),1,fp);
...
{
  if (comm->me == 0) {
    fread(&prefactor_global,sizeof(double),1,fp);
    fread(&mix_flag,sizeof(int),1,fp);
  }
  MPI_Bcast(&prefactor_global,1,MPI_DOUBLE,0,world);
...
void PairExcludedVolume::write_data(FILE *fp)
{
...
void PairExcludedVolume::write_data_all(FILE *fp)
{
  for (int i = 1; i <= atom->ntypes; i++)
                        ...
                        double &fforce)
{
  ...
  r = sqrt(rsq);
  arg = MY_PI*r/cut[itype][jtype];
  fforce = factor_lj * prefactor[itype][jtype] *  sin(arg) * cut[itype][jtype] / MY_PI / r;

  philj = prefactor[itype][jtype] * cutsq[itype][jtype] * (1.0+cos(arg)) / MY_PI / MY_PI;
  return factor_lj*philj;
}

/* ---------------------------------------------------------------------- */

void *PairExcludedVolume::extract(const char *str, int &dim)
{
  dim = 2;
