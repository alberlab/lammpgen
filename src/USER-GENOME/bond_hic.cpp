...
void BondHiC::compute(int eflag, int vflag)
{
  ...
  for (n = 0; n < nbondlist; n++) {
    ...
    r0 = range[type]*(radius[i1]+radius[i2]);
...
void BondHiC::coeff(int narg, char **arg)
{
  ...
  double range_one = force->numeric(FLERR,arg[2]);
...
void BondHiC::read_restart(FILE *fp)
{
  ...
  if (comm->me == 0) {
    ...
    fread(&range[1],sizeof(double),atom->nbondtypes,fp);
  }
  MPI_Bcast(&k[1],atom->nbondtypes,MPI_DOUBLE,0,world);
  MPI_Bcast(&range[1],atom->nbondtypes,MPI_DOUBLE,0,world);
                        ...
                        double &fforce)
{
  ...
  int index = atom->find_custom("radius", dflag);
  double *radius = atom->dvector[index];
  double r0 = range[type]*(radius[i] + radius[j]);
  if (r < r0) return 0;
  double dr = r - r0;
  double rk = k[type] * dr;
  fforce = 0;
